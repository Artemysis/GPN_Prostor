...
export function RequestHeaderForm({ request, onSaved }: { request: RequestRecord; onSaved?: () => void }) {
  ...
  const contracts = api.listContracts(watchedCompanyId || undefined)
  const products = api.listProducts(watchedContractId || undefined)
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      <div>
        <Label htmlFor="hf_title">
          Название заявки <FilledByBadge value={request.filled_by.title} />
        </Label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label htmlFor="hf_company">
            Подрядчик <FilledByBadge value={request.filled_by.company_id} />
          </Label>
          <Select
            id="hf_company"
            {...register('company_id')}
        ...
        <div>
          <Label htmlFor="hf_contract">
            Договор <FilledByBadge value={request.filled_by.contract_id} />
          </Label>
      ...
      <div>
        <Label htmlFor="hf_product">
          Продукт (услуга) <FilledByBadge value={request.filled_by.product_id} />
        </Label>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          <Label htmlFor="hf_cost">
            Стоимость, ₽ <FilledByBadge value={request.filled_by.cost_total} />
          </Label>
