...
export function TzStageEditor({
  ...
  const patchStage = (id: string, patch: Partial<TzStage>) => {
    void api.updateStage(requestId, id, patch).then(refresh)
  }

  const applyDraft = () => {
    if (!draft) return
    void Promise.all(
      draft.map((st, i) => api.addStage(requestId, { ...st, stage_order: tz.length + i + 1 })),
    ).then(() => {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/60 px-5 py-3">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            onClick={() => {
              setDraft(null)
              void api.startFillAi(requestId, 'work_content').then((job) => setJobId(job.id))
            }}
          >
              ...
              <div className="absolute right-0 top-full z-10 mt-1 w-80 rounded-xl border border-slate-200 bg-white p-2 shadow-card">
                ...
                {unusedSkeleton.map((s) => (
                  <button
                    ...
                    onClick={() => {
                      void api.addStage(requestId, { stage_name: s.stage_name, stage_order: tz.length + 1, requirements: '', expected_results: '' }).then(refresh)
                ...
                <button
                  ...
                  onClick={() => {
                    void api.addStage(requestId, { stage_name: `Этап ${tz.length + 1}`, stage_order: tz.length + 1 }).then((st) => {
                      setOpenId(st.id)
                      refresh()
                    })
                    setShowSkeleton(false)
            ...
            <div className="flex items-center justify-between gap-3 px-4 py-3">
              ...
              <div className="flex shrink-0 items-center gap-1">
                <button
                  ...
                  onClick={() => {
                    void api.deleteStage(requestId, s.id).then(refresh)
                  }}
                >
            ...
            {openId === s.id && (
              <div className="grid grid-cols-2 gap-3 border-t border-slate-100 px-4 py-3">
                ...
                <div>
                  ...
                  <Textarea rows={3} value={s.requirements} placeholder={template.stages.find((x) => x.stage_name === s.stage_name)?.default_requirements ?? undefined} onChange={(e) => patchStage(s.id, { requirements: e.target.value })} />
                </div>
                <div>
                  <Label>Ожидаемые результаты</Label>
                  <Textarea rows={3} value={s.expected_results} placeholder={template.stages.find((x) => x.stage_name === s.stage_name)?.default_results ?? undefined} onChange={(e) => patchStage(s.id, { expected_results: e.target.value })} />
                </div>
                <div className="col-span-2">
