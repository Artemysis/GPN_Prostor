...
import type { ChatAction, ChatMessage as ChatMessageType } from '@/lib/api'
...
function resolveValue(field: string | undefined, value: string | undefined): string {
  if (!field || value === undefined) return value ?? ''
  if (field === 'company_id') return api.listCompanies().find((c) => c.company_id === value)?.name ?? value
  if (field === 'product_id') return api.listProducts().find((p) => p.product_id === value)?.product_name ?? value
  if (field === 'contract_id') return api.listContracts().find((k) => k.contract_id === value)?.contract_number ?? value
...
function ActionsPanel({
  ...
  const template = templateAction ? api.listTemplates().find((t) => t.id === templateAction.template_id) : undefined
              ...
              >
                ...
                <span className="flex-1">
                  <span className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="font-semibold text-brand-800">{resolveValue(a.field, a.value)}</span>
                    {a.applied ? (
