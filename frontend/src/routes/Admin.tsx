...
const ingestItems = [
  { key: 'companies', file: '0. Компании.xlsx', label: 'Компании' },
  { key: 'contracts', file: '1. Договоры.xlsx', label: 'Договоры' },
  { key: 'calculations', file: '2. Договор + РС.xlsx', label: 'Договор + РС' },
  { key: 'products-rates', file: '3/4. Продукты + расценки.xlsx', label: 'Продукты и расценки' },
  { key: 'operations', file: '5. Продукты + Операции.xlsx', label: 'Операции продуктов' },
]
...
export default function Admin() {
  ...
  const ingest = async (key: string, file: File | undefined) => {
    if (!file) return
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 px-6 py-8">
              ...
              <div key={item.key} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 px-4 py-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    accept=".xlsx"
                    className="hidden"
                    onChange={(e) => void ingest(item.key, e.target.files?.[0])}
                  />
        ...
        <CardContent>
          <div className="flex items-center justify-between gap-3">
            ...
            <Button
              ...
              onClick={() => {
                const job = api.rebuildEmbeddings()
                setEmbeddingJobId(job.id)
              }}
            >
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-slate-600">Сброс к исходному состоянию мок-базы (справочники, заявки, ТЗ, чаты, документы).</p>
            <Button
              variant="destructive"
              onClick={() => {
                resetDemoData()
