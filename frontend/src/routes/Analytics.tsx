  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4 px-6 py-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Типов ТЗ</p>
            <BarChart3 className="h-4 w-4 text-brand-700" />
          </div>
          <p className="mt-2 text-3xl font-bold text-slate-900">{tz.by_type.length}</p>
        </Card>
        <Card className="p-5">
          <div className="flex items-center justify-between">
          ...
          <p className="mt-2 truncate text-lg font-bold text-slate-900">{search.top_services[0]?.product_name ?? '—'}</p>
          ...
          <CardContent className="space-y-3">
            {search.top_contractors.map((c) => (
              <div key={c.name} className="flex items-center justify-between gap-2">
                <p className="min-w-0 truncate text-sm font-medium text-slate-800">{c.name}</p>
          ...
          <CardContent className="space-y-2.5">
            {search.unfilled_fields.map((f) => (
              <div key={f.field} className="flex items-center gap-2">
                ...
                <span className="shrink-0 text-xs font-bold text-slate-500">{f.empty_count}</span>
              </div>
            ))}
          </CardContent>
        </Card>
